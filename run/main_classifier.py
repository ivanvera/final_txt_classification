...
class MainClassifier(object):
    def __init__(self):
        # 主要都是直接从配置文件里读取
        # 加载类别与id的映射字典
        self.category_dic, self.category_reverse_dic = self.load_category_dic_from_pkl()
        # 加载词典
    ...
    def load_category_dic_from_pkl(self):
        return Util.load_object_from_pkl(FilePathConfig.category_pkl_path), \
               Util.load_object_from_pkl(FilePathConfig.category_reverse_pkl_path)
...
def main2():
    ...
    for key, value in mainClassifier.category_reverse_dic.items():
        print key, value
    # 只返回单分类
    classify_result = mainClassifier.classify_documents(feature_mat)
    print mainClassifier.category_reverse_dic[int(classify_result[0][0][0])]
    # 需要返回的类别数量
    k = 3
    # # 返回多个分类和其概率
